...
options.register('pumbins', 18, VarParsing.multiplicity.singleton, VarParsing.varType.int, 'Number of pileupbins to process')
...
process.source.lumisToProcess = LumiList.LumiList(filename = 'Run2016C.json').getVLuminosityBlockRange()


outputFile = '/data/' + os.environ['USER'] + '/UCTPUMTable-' + str(options.runNumber) + '-'+ str(options.pumbins)+'.root'
...
process.PUMcalcCentralBX = cms.EDAnalyzer("PUMcalc",
    ...
    pumbins = cms.untracked.int32(options.pumbins),
